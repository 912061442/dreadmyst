...
void Game::processPacket_Server_NewWorld(StlBuffer& data)
{
	...
	{
		world->setMap(pk.m_mapId);

		if (auto gameChat = dynamic_pointer_cast<GameChat>(world->getRenderObject(World::GameChatBox)))
		{
			gameChat->addLine("Welcome to Dreadmyst. Join our Discord at dreadmyst.com for help.", ChatDefines::Channels::System);
			gameChat->addLine("Read and follow our rules located at dreadmyst.com/rules", ChatDefines::Channels::System);
		}
	}
}
...
void Game::processPacket_Server_Inventory(StlBuffer& data)
{
	...
	{
		const auto& slot = pk.m_slots[i];
		auto icon = dynamic_pointer_cast<ItemIcon>(inventory->getRenderObject(Inventory::Interface::Slot1 + i));
		icon->setItemDef(slot.itemId);
		icon->setStackCount(slot.stackCount);
...
void Game::processPacket_Server_CombatMsg(StlBuffer& data)
{
		...
		!incoming.m_periodic)
	{
		...
		sf::Color textColor = incoming.m_positive ? sf::Color(87, 193, 137, 255) : sf::Color::Red;
		float scale = 0.5f;
