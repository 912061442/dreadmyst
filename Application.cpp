...
string Application::getCacheFolderPath() const
{
	...
	if (result.empty())
	{
		...
		// Try to use %APPDATA%\Dreadmyst\config.ini
		if (SUCCEEDED(SHGetFolderPathA(NULL, CSIDL_APPDATA, NULL, 0, appDataPath)))
		{
			std::string appDataDir = std::string(appDataPath) + "/Dreadmyst/";
...
void Application::createSfmlWindow()
{
	...
	std::string windowName = sConfig->getString("Window", "WindowName", "Application");
...
void Application::setFullscreenBorderless(const bool val)
{
	...
	if (m_isFullscreen)
	{
		...
		// Create as Default, then strip decorations
		m_window.create(sf::VideoMode(monW, monH), "Dreadmyst", sf::Style::Default, m_contextSettings);
		m_window.setVerticalSyncEnabled(false);
		ImmAssociateContext(m_window.getSystemHandle(), NULL);
	...
	else
	{
		...
		m_window.create(windowed, "Dreadmyst", sf::Style::Default, m_contextSettings);
		m_window.setVerticalSyncEnabled(false);
		ImmAssociateContext(m_window.getSystemHandle(), NULL);
