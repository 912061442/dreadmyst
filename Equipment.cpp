...
void Equipment::setStatView(const StatView view)
{
	...
	{
		attachLabel(ObjDefines::Variable::StatsStart + UnitDefines::Stat::Health, { labelsX, 140 });
		...
		attachValue(ObjDefines::Variable::StatsStart + UnitDefines::Stat::Health, { valuesX, 140 }, TextBox::AlignLeft);
	...
	attachValue(ObjDefines::Variable::Progression, { 191, 248 }, TextBox::AlignCenterAbsolute);
	attachValue(ObjDefines::Variable::Experience, { 191, 303 }, TextBox::AlignCenterAbsolute);

	if (m_spendingPoints)
	{
		attachValue(CustomLabel::LevelupCost, { 191, 358 }, TextBox::AlignCenterAbsolute);
	}
...
shared_ptr<TextBoxRo> Equipment::attachValue(const int var, const sf::Vector2i& offset, const int alignment, const bool spendButtons /*= true*/)
{
	auto textRo = make_shared<TextBoxRo>(*this, Interface::ValuesKey + var, "Palatino Linotype Regular.ttf", 500, 14, TextBox::Align(alignment), true, 2.f);
	textRo->setAnchor(&getTopLeftCornerRef());
	textRo->setOffset(offset);
	addRenderObject(textRo);
	...
	if (spendButtons)
	{
		attachSpendButton(var, offset + sf::Vector2i(35, -7));
		attachSpendMinButton(var, offset + sf::Vector2i(55, -7));
	}
