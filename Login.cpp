	...
	RenderObjectHolder(&owner, id)
{
	...
	if (defaultLogin.size() <= 1)
		sApplication->spawnPopup("To play this game, you must first Register an account at https://dreadmyst.com/.", ConfirmMessageBox::ConfirmBox_Ok, 0);
...
void Login::render()
{
	if (m_loginAttemptAt && ::clock() > m_loginAttemptAt)
	{
		...
		// TLS authetnication
		if (!sConnector->attemptAuth(L"dreadmyst.com", L"/auth/auth.php", userStr, passwordStr, token, error))
		{
